...
- Summarize Document: generates a summary of the document content.
- Opinion / Critique: provides a critical analysis of the document (strengths, weaknesses, recommendations) grounded in the document text.
...
if not os.getenv("OPENAI_API_KEY"):
    st.error("OPENAI_API_KEY is missing. Configure it in Streamlit Secrets (same as Exercise 1).")
    st.stop()
...
chunk_size = st.sidebar.slider("Chunk size (text length per segment)", 300, 1500, 900, 100)
chunk_overlap = st.sidebar.slider("Chunk overlap (context continuity)", 0, 400, 120, 20)

show_context = st.sidebar.checkbox("Show retrieved context (debug)", value=False)
faithfulness_check = st.sidebar.checkbox("Evaluate faithfulness (LLM-as-judge)", value=False)

st.sidebar.divider()
st.sidebar.subheader("External Knowledge Base (PDF)")
uploaded_pdf = st.sidebar.file_uploader("Upload a PDF document (optional)", type=["pdf"])
...
def build_vectorstore_from_pdf_bytes(pdf_bytes: bytes, chunk_size: int, chunk_overlap: int):
    ...
    return db, text, chunks


if uploaded_pdf is not None:
    pdf_bytes = uploaded_pdf.read()
    db, doc_text, chunks = build_vectorstore_from_pdf_bytes(pdf_bytes, chunk_size, chunk_overlap)
    ...
    st.sidebar.success(f"PDF indexed: {len(chunks)} chunks | ~{st.session_state.doc_words} words")
...
def summarize_document(style: str, max_words: int | None):
    if not st.session_state.doc_text.strip() or not st.session_state.chunks:
        return "No readable text was extracted from the PDF. If the PDF is scanned (image-based), OCR is required.", 0

    chunks = st.session_state.chunks[:min(len(st.session_state.chunks), 8)]

    partial_summaries = []
    for i, ch in enumerate(chunks, start=1):
        p = f"""
Summarize the following part of a document. Keep it short and factual.
...
SHORT SUMMARY:
""".strip()
        partial_summaries.append(llm.invoke(p).content)

    constraint = f"Limit the summary to at most {max_words} words." if max_words else ""
    final_prompt = f"""
You are a helpful assistant.
Create a {style} summary of the document based ONLY on the partial summaries below.
{constraint}
...
if mode == "Summarize Document":
    ...
    with col2:
        summary_max_words = st.number_input("Max words (optional)", min_value=0, value=180, step=20)
...
def respond(user_input: str):
    ...
    if mode == "General Chat (no document)" or st.session_state.db is None:
        ...
        style = summary_style or "Concise"
        answer, used_rag = summarize_document(style, summary_max_words)
...
if user_input:
    ...
    with st.chat_message("assistant"):
        if st.session_state.db is None and mode != "General Chat (no document)":
